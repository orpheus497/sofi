...
| `sofi -show display` | `wlr-randr`, plus `backlight` (FreeBSD base) for brightness | Needs a compositor that advertises `wlr-output-management-unstable-v1` — hikari-sakura does as of its commit `60075bd`, as do other wlroots compositors. Without `wlr-randr` the pane says so rather than showing an empty list. **Brightness is the internal panel only**: `backlight(8)` writes `/dev/backlight/backlight0`, which is `root:video`, so add your user to the `video` group and no privilege is needed. External monitors need DDC/CI (`ddcutil`), which sofi does not drive |
